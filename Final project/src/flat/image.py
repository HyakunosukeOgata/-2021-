...
class bmp(object):
    ...
    def __init__(self, path):
        ...
        self.width = bmp_image.width
        self.height = bmp_image.height
        self.kind = 'rgb'

        file_bytes = np.array(bmp_image).reshape(-1)
        data = bytearray(self.width*self.height*3)
        offset = len(file_bytes) - len(data)
        for i in range(len(data)):
            data[i] = file_bytes[offset+i]
        self.data = data
...
class image(object):
    ...
    def decompress(self):
        if self.source:
            ...
            if 0:
                bmp_image = Image.open('IMG_2274_small.bmp')
                file_bytes = np.array(bmp_image).reshape(-1)
                data = bytearray(self.width*self.height*3)
                offset = len(file_bytes) - len(data)
                for i in range(len(data)):
    ...
    def jpeg(self, path='', DHT=None, DQT=None, Huffman_Optimized=False):
        #if isinstance(self.source, jpeg):
        #    return save(path, self.source.readable.data)
        self.decompress()

        data, huffman_optmized_tables = jpegserialize(self, DHT, DQT, Huffman_Optimized)
        if Huffman_Optimized:
            len_data_old = len(data)
            DHT = huffman_optmized_tables
            data, _ = jpegserialize(self, DHT, DQT, Huffman_Optimized)
            print(f'Huffman_Optimized, before:{len_data_old} after:{len(data)}')
