...
class huffman_counter(object):
    ...
    def count(self, used_info):
        ...
        return
    def print_optimized_huffman(self):
        ...
        print('lengths:bytes(', [ _ for _ in self.lengths], ')')
        print('values:bytes(', optimized_values, ')')
        return

def serialize(image, quality, DHT, DQT):
    ...
    lq = _quantization_table(encode_luminance_quantization, quality)
    ld = _huffman_table(encode_ld_lengths, encode_ld_values)
    la = _huffman_table(encode_la_lengths, encode_la_values)
    ls = _scale_factor(lq)
    if n == 3:
        cq = _quantization_table(encode_chrominance_quantization, quality)
    ...
    print("ld optimized:")
    huff_counter_ld.print_optimized_huffman()
    print("la optimized:")
    huff_counter_la.print_optimized_huffman()
    print("cd optimized:")
    huff_counter_cd.print_optimized_huffman()
    print("ca optimized:")
    huff_counter_ca.print_optimized_huffman()
