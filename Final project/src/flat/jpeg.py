...
class _entropy_encoder(object):
    ...
    def encode(self, previous, block, scale, dc, ac):
        ...
        if n > 0:
            self.write(*ac[0])
            ac_used.append(0)
        return block[0], dc_used, ac_used
...
class huffman_counter(object):
    def __init__(self, lengths, values):
        self.lengths = lengths
        self.values = values
        self.count_array = [0] * (max(values) + 1)
    def count(self, used_info):
        for i in used_info:
            self.count_array[i] += 1
        return
    def get_optimized_huffman(self):
        dtype = [('value', int), ('freq', int)]
        value_freq = np.array([(_, self.count_array[_]) for _ in self.values], dtype=dtype)
        value_freq_sorted = np.sort(value_freq, order='freq')[::-1]
        optimized_values = [_[0] for _ in value_freq_sorted]
        return [_ for _ in self.lengths], optimized_values


def serialize(image, DHT, DQT, Huffman_Optimized):
    encode_ld_lengths, encode_ld_values = _ld_lengths, _ld_values
    encode_la_lengths, encode_la_values = _la_lengths, _la_values

    encode_cd_lengths, encode_cd_values = _cd_lengths, _cd_values
    encode_ca_lengths, encode_ca_values = _ca_lengths, _ca_values

    encode_luminance_quantization = _luminance_quantization
    encode_chrominance_quantization = _chrominance_quantization
    if 1:
        if DHT:
            if DHT['_ld_lengths']:
                encode_ld_lengths, encode_ld_values = DHT['_ld_lengths'], DHT['_ld_values']
            if DHT['_la_lengths']:
                encode_la_lengths, encode_la_values = DHT['_la_lengths'], DHT['_la_values']
            if DHT['_cd_lengths']:
                encode_cd_lengths, encode_cd_values = DHT['_cd_lengths'], DHT['_cd_values']
            if DHT['_ca_lengths']:
                encode_ca_lengths, encode_ca_values = DHT['_ca_lengths'], DHT['_ca_values']
        if DQT:
            if DQT['luma']:
                encode_luminance_quantization = DQT['luma']
            if DQT['chroma']:
                encode_chrominance_quantization = DQT['chroma']
    ...
    #lq = _quantization_table(encode_luminance_quantization, quality=100)
    lq = encode_luminance_quantization
    ld = _huffman_table(encode_ld_lengths, encode_ld_values)
    la = _huffman_table(encode_la_lengths, encode_la_values)
    ls = _scale_factor(lq)
    if n == 3:
        #cq = _quantization_table(encode_chrominance_quantization, quality=100)
        cq = encode_chrominance_quantization
        cd = _huffman_table(encode_cd_lengths, encode_cd_values)
        ca = _huffman_table(encode_ca_lengths, encode_ca_values)
        cs = _scale_factor(cq)

    e = _entropy_encoder()

    huff_counter_ld = huffman_counter(_ld_lengths, _ld_values)
    huff_counter_la = huffman_counter(_la_lengths, _la_values)
    huff_counter_cd = huffman_counter(_cd_lengths, _cd_values)
    huff_counter_ca = huffman_counter(_ca_lengths, _ca_values)
            ...
            for yy in range(y, y + 8):
                for xx in range(x, x + 8):
                    ...
                    if n == 1:
                        ...
                        kblock[i] = data[j + 3]
                    i += 1
            ydc, dc_used, ac_used = e.encode(ydc, yblock, ls, ld, la)
            huff_counter_ld.count(dc_used)
            huff_counter_la.count(ac_used)
            if n == 3:
                udc, dc_used, ac_used = e.encode(udc, ublock, cs, cd, ca)
                huff_counter_cd.count(dc_used)
                huff_counter_ca.count(ac_used)
                vdc, dc_used, ac_used = e.encode(vdc, vblock, cs, cd, ca)
                huff_counter_cd.count(dc_used)
                huff_counter_ca.count(ac_used)
            elif n == 4:
                udc, dc_used, ac_used = e.encode(udc, ublock, ls, ld, la)
                vdc, dc_used, ac_used = e.encode(vdc, vblock, ls, ld, la)
                kdc, dc_used, ac_used = e.encode(kdc, kblock, ls, ld, la)


    ld_optimized_huffman = huff_counter_ld.get_optimized_huffman()
    #print("ld optimized:")
    #print('lengths:bytes(', ld_optimized_huffman[0], ')')
    #print('values:bytes(', ld_optimized_huffman[1], ')')

    la_optimized_huffman = huff_counter_la.get_optimized_huffman()
    #print("la optimized:")
    #print('lengths:bytes(', la_optimized_huffman[0], ')')
    #print('values:bytes(', la_optimized_huffman[1], ')')

    cd_optimized_huffman = huff_counter_cd.get_optimized_huffman()
    #print("cd optimized:")
    #print('lengths:bytes(', cd_optimized_huffman[0], ')')
    #print('values:bytes(', cd_optimized_huffman[1], ')')

    ca_optimized_huffman = huff_counter_ca.get_optimized_huffman()
    #print("ca optimized:")
    #print('lengths:bytes(', ca_optimized_huffman[0], ')')
    #print('values:bytes(', ca_optimized_huffman[1], ')')

    DHT = {
        '_ld_lengths':bytes( ld_optimized_huffman[0] ),
        '_ld_values':bytes( ld_optimized_huffman[1] ),
        '_la_lengths':bytes( la_optimized_huffman[0] ),
        '_la_values':bytes( la_optimized_huffman[1] ),

        '_cd_lengths':bytes( cd_optimized_huffman[0] ),
        '_cd_values':bytes( cd_optimized_huffman[1] ),
        '_ca_lengths':bytes( ca_optimized_huffman[0] ),
        '_ca_values':bytes( ca_optimized_huffman[1] ),
    }
    ...
    dqt = b'\0' + lq
    dht = b'\0' + encode_ld_lengths + encode_ld_values + b'\20' + encode_la_lengths + encode_la_values
    if n == 3:
        sof += b'\2\21\1\3\21\1'
        sos += b'\2\21\3\21'
        dqt += b'\1' + cq
        dht += b'\1' + encode_cd_lengths + encode_cd_values + b'\21' + encode_ca_lengths + encode_ca_values
    elif n == 4:
